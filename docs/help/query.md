...
- **Plan view** — top-down tree of operator boxes connected by T-junction lines. Each box shows the operator's `exec` time (its own non-overlapping CPU work) and turns **orange** when that operator accounts for more than 50% of the plan's total operator CPU — that's your bottleneck. The percent badge next to the time is `exec / planTotalExec` (share of plan operator work), so the badges across all visible boxes sum to 100% and you can read which operator owns which fraction of the CPU at a glance.
