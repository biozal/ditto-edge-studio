...
fun DatabaseCard(
    ...
    // Delete is irreversible and destroys more than the card suggests: the config row's
    // `databaseId` is the parent key four child tables cascade from, so it takes every
    // subscription, observer, favorite and query-history row with it. The menu is opened by
    // long-press — the same gesture a scroll-and-hold produces — and Delete sits next to
    // Edit, so an unconfirmed single tap was losing the user's whole working set.
    // macOS/iPadOS has always gated this (DatabaseDeletionConfirmation.swift); this is the
    // Android half of that gate, with the same wording.
    if (showDeleteConfirmation) {
        AlertDialog(
            onDismissRequest = { showDeleteConfirmation = false },
            title = { Text("Delete ${database.name}?") },
            text = {
                Text(
                    "This deletes the local database and all its Edge Studio data. " +
            ...
            confirmButton = {
                TextButton(
                    ...
                    modifier = Modifier.testTag("ConfirmDeleteDatabaseButton"),
                ) {
