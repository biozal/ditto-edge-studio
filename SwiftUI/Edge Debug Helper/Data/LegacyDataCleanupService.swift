    ...
    func cleanupLegacyData() async throws {
        ...
        if fileManager.fileExists(atPath: oldConfigDir.path) {
            do {
                ...
                Log.info("Deleted old config directory: \(oldConfigDir.path)")
            } catch {
                Log.error("Failed to delete old config directory: \(error.localizedDescription)")
                errors.append(error)
            }
        }
        ...
        if fileManager.fileExists(atPath: oldAppsDir.path) {
            do {
                ...
                Log.info("Deleted old apps directory: \(oldAppsDir.path)")
            } catch {
                Log.error("Failed to delete old apps directory: \(error.localizedDescription)")
                errors.append(error)
            }
        }
        ...
        Log.info("Legacy data cleanup completed successfully")
    }
